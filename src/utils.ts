...
export function calculateRiskScore(data: WeatherData): number {
  ...
  if (data.humidity < ASTHMA_THRESHOLDS.HUMIDITY_LOW) {
    ...
    data.humidity >= ASTHMA_THRESHOLDS.HUMIDITY_OPTIMAL_MIN &&
    data.humidity <= ASTHMA_THRESHOLDS.HUMIDITY_OPTIMAL_MAX
  ) {
    score += 0; // Optimal
  ...
  if (data.temperature < ASTHMA_THRESHOLDS.TEMP_COLD) {
    ...
    data.temperature >= ASTHMA_THRESHOLDS.TEMP_OPTIMAL_MIN &&
    data.temperature <= ASTHMA_THRESHOLDS.TEMP_OPTIMAL_MAX
  ) {
    score += 0; // Optimal
