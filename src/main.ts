...
const recommendationsContainerEl = document.getElementById(
...
const citySearch = document.getElementById(
  "city-search"
...
function renderForecast(forecastData: any, aqiData: any) {
  ...
  for (
    let i = 0;
    i < hoursToShow && i < forecastData.hourly.time.length;
    i += step
  ) {
    ...
    forecastEl.innerHTML = `
      ...
      <div class="forecast-aqi" style="color: ${aqiLevel.color}">${Math.round(
      aqi
    )}</div>
...
async function handleFetch(
  ...
  try {
    ...
    showError(
      "Não foi possível carregar os dados meteorológicos. Verifique sua conexão."
    );
  }
}
...
const handlePosition: PositionCallback = (position) => {
  ...
  handleFetch(latitude, longitude, "Sua Localização", "BR").catch((err) =>
...
const handlePositionError: PositionErrorCallback = (err) => {
  console.warn(
    "Geolocation unavailable or denied, falling back to default coordinates",
    err
  );
  handleFetch().catch((e) =>
    console.error("Error fetching weather with default coords", e)
  );
};
...
function initApp() {
  ...
  if (savedLocation) {
    ...
    if (typeof navigator !== "undefined" && navigator.geolocation) {
      ...
      console.warn(
        "Navigator.geolocation not available, using default coordinates"
      );
      handleFetch().catch((e) =>
        console.error("Error fetching weather with default coords", e)
      );
...
function renderCities(cities: ICity[]) {
  ...
  if (cities.length === 0) {
    cityListContainer.innerHTML =
      '<p class="no-cities-text">Nenhuma cidade encontrada</p>';
    return;
  }
...
function resetCities() {
  if (!cityListContainer || !citySearch) return;
  cityListContainer.innerHTML =
    '<p class="hint-text">Selecione um país e estado primeiro</p>';
